...
export default class MicroChildApp {
  ...
  __handleIframeInitResponseConfigMessage (message: MessageEntity) {
    ...
    // 设置钩子事件 - iframe初始化成功
    mittBus.emit('iframe-ready', message)
  }
  ...
  __init (microName: string, microOptions: MicroIFrameOptionsType) {
    ...
    // 发送消息到主应用，获取配置
    const HandShakeMessage = new MessageEntity({ fromApp: this.MICRO_IFRAME_NAME, targetApp: '', type: MicroChildApp.MICRO_IFRAME_EVENT.INIT_REQUEST_CONFIG, data: undefined })
    ...
    window.addEventListener('message', (event: MessageEvent) => {
      ...
      if (event?.data?.type === MicroChildApp.MICRO_IFRAME_EVENT.INIT_RESPONSE_CONFIG) {
        this.__handleIframeInitResponseConfigMessage(event?.data || {})
        return
      }
